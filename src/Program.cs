...
namespace AK.Homepage
{
	public static class Program
	{
		...
		{
			var runningInContainer = bool.TryParse(Environment.GetEnvironmentVariable("AK_RUNNING_IN_CONTAINER"), out var b) ? b : false;
			var isDevelopment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development";
			var listenPort = int.TryParse(Environment.GetEnvironmentVariable("AK_LISTEN_PORT"), out var p) ? p : 5858;

			var builder = Host.CreateDefaultBuilder(args).ConfigureWebHostDefaults(w =>
			{
				w.UseStartup<Startup>();
				if (isDevelopment || runningInContainer) w.UseKestrel(o => o.Listen(IPAddress.Any, listenPort));
			});
