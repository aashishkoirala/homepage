...
namespace AK.Homepage.Blog
{
    public class BlogCache
    {
        ...
        private PostLink[] GetHomePostLinks(Cache cache, Category category)
        {
	        ...
	        {
		        links = links
			        .Concat(Enumerable.Range(0, _homeLinkCount - links.Length).Select(x => new PostLink {IsFiller = true}))
			        .ToArray();
	        }
        ...
        private class Cache
        {
            public IDictionary<string, PostCache> PostsBySlug { get; set; } = new Dictionary<string, PostCache>();
            public IDictionary<Category, PostLink[]> PostLinksByCategory { get; set; } = new Dictionary<Category, PostLink[]>();
            public PostLink[] HomeTechPostLinks { get; set; } = new PostLink[0];
            public PostLink[] HomeNonTechPostLinks { get; set; } = new PostLink[0];
        }
    }
}