...
namespace AK.Homepage.Blog
{
    public class PostInfoUrlManager
    {
        ...
        public PostInfo? UrlToPostInfo(string relativeMarkdownUrl)
        {
            ...
            var parts = relativeMarkdownUrl[0..^3].Split('/');
        ...
        {
	        var tags = postInfo.Tags.Any() ? $"@{string.Join(",", postInfo.Tags)}" : string.Empty;
	        return
        ...
        {
            var slug = title;
            foreach (var (key, value) in _slugifyReplacementMap) slug = slug.Replace(key, value);
